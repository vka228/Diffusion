...
class Space:
    ...
    def update(self):
        ...
        x0 = self._helium.getx()
        y0 = self._helium.gety()
        for mol in self._mol:
        ...
        x1 = self._helium.getx()
        y1 = self._helium.gety()
        sq_off.append((dist_btw(x0, y0, x1, y1) ** 2))
        c_diff.append(np.mean(sq_off) / (4 * 10 ** -6))
